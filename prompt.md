# 1

帮我编写一个 ckb 区块链的浏览器钱包插件. 功能

1. 允许导入或自动生成私钥.
    1. 导入的私钥以 16 进制表示, 不要使用助记词.
    2. 生成钱包时随机生成密码学安全的私钥.
2. 允许导出私钥.
3. 可查看账户余额.
4. 可在测试网上进行转账交易.

# 2

目前钱包用的是测试网默认的 secp256k1 签名(code_hash: 9bd7e06f3ecf4be0f2fcd2188b23f1b9fcc88e5d4b65a8637b17723bbda3cce8), 后续我想支持其它类型的签名算法, 你目前做以下几个修改:

1. 导入或者生成私钥的时候, 账户类型是可选的. 目前可以只支持一个账户类型, 叫做 secp256k1 就可以了; 用户解锁钱包后, 会在地址后面显示账户类型, 并且可以修改账户类型.

# 3

阅读 https://delvingbitcoin.org/t/shrincs-324-byte-stateful-post-quantum-signatures-with-static-backups/2158, 我想尝试在 ckb 钱包里实现 shrincs 签名. 账户类型增加 shrincs, 但是由于 shrincs 目前尚未在测试网部署, 你可以假设 shrincs 脚本的 code_hash, cell_dep 的 out_point 的 tx_hash 等数据都为零.

- 尽量使用 stateful 签名, 因为该签名的 signature 较小. 尽量不用 stateless 签名, 签名很大.
- 当导入 secret key 时, 需要将该 key 的 stateful 签名禁止掉. 因为不清楚导出时, 该 key 是否使用了 stateful 签名.
- stateless 签名时, 耗时较长, 需要在 UI 上提示, 同时提供进度条

# 4

重构页面布局, 提升用户体验. 页面描述:

- 创建账户页面. 提供两个按钮, 分别是生成随机账户, 导入已有账户. 点击后跳转到对应的操作页面.
- 生成随机账户. 有下拉框可选账户类型, 分别是 secp256k1 和 shrincs-rs 账户. 有密码输入框, 用于设置账户密码. 有确定按钮, 点击后跳转到账户页面.
- 导入已有账户. 有下拉框可选账户类型, 分别是 secp256k1 和 shrincs-rs 账户. 有密码输入框, 用于设置账户密码. 有私钥输入框, 用于输入私钥. 有确定按钮, 点击后跳转到账户页面.
- 账户页面. 从上往下, 分别是显示地址, 显示地址类型, 显示可用余额, 余额刷新按钮. 然后下面是两个可切换的子页面, 分别是
    - 发起交易页面. 包含目标地址和金额输入框, 签名并发送按钮.
    - 交易历史页面. 显示交易记录列表, 包含交易哈希, 交易状态, 交易时间, 交易金额等信息.
- 设置页面. 从上往下, 分别是选择语言(中英文), 备份私钥按钮, 签名类型下拉框, 锁定钱包按钮. 对于 secp256k1 账户, 签名类型下拉框只显示并可选默认; 对于 shrincs-rs 账户, 签名类型下拉框显示并可选有状态化(stateful)和无状态化(stateless).

插件允许在边栏中打开.

# 5

- shrincs 账号使用 CKB_TX_MESSAGE_ALL 方案. spec 描述: https://github.com/nervosnetwork/rfcs/pull/446. 你需要在 js 里实现 CKB_TX_MESSAGE_ALL 方案, 然后仅在 shrincs 账户进行签名时使用该方案.
- 目前 shrincs 算法已经部署在 testnet, 可以在测试网上进行实际的签名和交易操作, 移除关于实验性, 空占位之类的描述. 部署信息如下:

| parameter |                               value                                |
| --------- | ------------------------------------------------------------------ |
| code_hash | 0x387496fafe46562bb3bb2fa4446f1fc1054ba2f1b4df229a88056d5422a196ac |
| hash_type | type                                                               |
| tx_hash   | 0x3216d00b72e8229d7dbb46a93ea47bd0c650f2bdae42be2f92837328413da48e |
| index     | 0x0                                                                |
| dep_type  | code                                                               |

# 6

增加一个欢迎页面. 当用户首次打开钱包时, 显示欢迎信息, 并提供"开始使用"按钮, 点击后跳转到设置密码页面.

增加一个设置密码页面. 有两个输入框, 分别是密码和二次输入密码. 有确定按钮, 点击后跳转到创建账户页面. 有返回按钮, 点击后返回欢迎页面.

在"生成随机账户"和"导入已有账户"页面中, 不再需要用户输入密码.

# 7

允许多账户切换.

1. 在设置页面里, 增加"创建账户"按钮, 点击后跳转到创建账户页面. 创建账户页面复用已有的创建账户页面布局, 也就是提供两个按钮, 分别是生成随机账户, 导入已有账户. 点击后跳转到对应的操作页面.
2. 在账户页面里, 增加"切换账户"按钮, 点击后弹出账户列表, 选择后切换到对应账户.

# 8

创建一个测试网页, 这个网页会向插件请求签名, 网页会构建一个向  ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqdrcaufs8qeu8wvvy0myyedek4vqad9qeq3gc4cf 转账 128 ckb 的交易.

# 页面描述

- 欢迎页面. 当用户首次打开钱包时, 显示欢迎信息, 并提供"开始使用"按钮, 点击后跳转到设置密码页面.
- 设置密码页面. 有两个输入框, 分别是密码和二次输入密码. 有确定按钮, 点击后跳转到创建账户页面. 有返回按钮, 点击后返回欢迎页面.
- 输入密码页面. 有一个输入框, 用于输入密码. 有确定按钮, 点击后跳转到账户页面. 如果用户已经设置过密码, 那么每次打开钱包时都会弹出该页面要求输入密码.
- 创建账户页面. 提供两个按钮, 分别是生成随机账户, 导入已有账户. 点击后跳转到对应的操作页面.
- 删除账户页面. 返回按钮位于页面左上角, 有确认删除按钮, 点击后删除当前账户. 如果用户还有其它账户, 删除后会进入账户选择页面, 否则会返回创建账户页面.
- 生成随机账户. 有下拉框可选账户类型, 分别是 secp256k1 和 shrincs-rs 账户. 有确定按钮, 点击后跳转到账户页面.
- 导入已有账户. 有下拉框可选账户类型, 分别是 secp256k1 和 shrincs-rs 账户. 有私钥输入框, 用于输入私钥. 有确定按钮, 点击后跳转到账户页面.
- 账户页面. 从上往下, 分别是显示地址, 显示地址类型, 显示可用余额, 余额刷新按钮. 然后下面是两个可切换的子页面, 分别是
    - 发起交易页面. 包含目标地址和金额输入框, 签名并发送按钮.
    - 交易历史页面. 显示交易记录列表, 包含交易哈希, 交易状态, 交易时间, 交易金额等信息.
- 设置页面. 从上往下, 分别是选择语言(中英文), 备份私钥按钮, 签名类型下拉框, 锁定钱包按钮. 对于 secp256k1 账户, 签名类型下拉框只显示并可选默认; 对于 shrincs-rs 账户, 签名类型下拉框显示并可选有状态化(stateful)和无状态化(stateless).
